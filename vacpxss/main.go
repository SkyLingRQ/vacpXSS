...
func xsScan(url string, payload string, wg *sync.WaitGroup, sem chan struct{}, user_agent string, x *int) {
	defer wg.Done()
	sem <- struct{}{}
	defer func() { <-sem }()
	...
	if strings.Contains(string(HTMLContent), payload) {
		fmt.Println("\033[31m[ POSSIBLE VULNERABLE ] " + url + "\033[0m")
		*x = *x + 1
	}
...
func main() {
	...
	file := flag.String("file", "urls.txt", "Ruta del archivo con URLS.")
	sem := flag.Int("sem", 50, "Implementar semaforo personalizado.")
	flag.Parse()
	...
	if err != nil {
		fmt.Println("[ ERROR ] Failed reading file " + *file + ".")
		return
	}
	...
	for bufioScanner.Scan() {
		...
		for _, payload := range payloads {
			...
			wg.Add(1)
			go xsScan(urlParsed.String(), payload, &wg, semaphore, user_agent, &contador)
		}

	}
	wg.Wait()
	fmt.Printf("\n\033[32m[+] Scan Results: %d xss found.\033[0m\n", contador)
}
